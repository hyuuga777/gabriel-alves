...
import { Mail, ArrowLeft, CheckCircle2, Loader2, Dumbbell } from 'lucide-react';
...
export default function EsqueciSenhaPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setMessage(data.message || 'E-mail de redefinição enviado com sucesso!');
        setEmail('');
      } else {
    ...
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden py-12 px-6">
      ...
      >
        ...
        <div className="glass rounded-[2rem] p-8 border border-white/5 shadow-2xl relative overflow-hidden">
          ...
          <h1 className="text-3xl font-black text-white mb-2 tracking-tight">Recuperar Senha</h1>
          <p className="text-gray-400 mb-8 text-sm leading-relaxed">
            Digite o seu e-mail de acesso e nós lhe enviaremos as instruções para redefinir sua senha.
          </p>
          ...
          <AnimatePresence mode="wait">
            {message ? (
              <motion.div
                ...
                className="bg-primary/10 border border-primary/20 p-6 rounded-2xl flex flex-col items-center text-center gap-4 mb-6"
              >
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                >
                  {loading ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" /> Enviando Link...
                    </>
                  ) : (
