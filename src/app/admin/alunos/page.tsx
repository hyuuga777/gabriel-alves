...
export default function AdminAlunosPage() {
    ...
    const fetchData = useCallback(async () => {
        try {
            ...
            const filteredPlanos = planosData.filter((p: any) => 
    ...
    const handleCreateStudent = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                fetchData(); // Changed from fetchAlunos() to fetchData()
            } else {
                const errorData = await res.json().catch(() => ({})); // Changed from response.json() to res.json()
                throw new Error(errorData.error || 'Falha ao criar aluno');
            }
        } catch (error: any) {
            console.error(error);
            alert(error.message || 'Erro ao criar aluno. Verifique os dados e tente novamente.');
        } finally {
